...
def is_zero_velocity(acc_x, acc_y, acc_z, gyro_x, gyro_y, gyro_z):
    ...
    # 加速度扰动阈值：判断是否受运动干扰
    ACC_VARIANCE_THRESHOLD = 0.05 * 9.81  # 小幅抖动视为静止
...
def update_buffers(data):
    absolute_acc = 9.81 * remove_gravity(data.acc_x, data.acc_y, data.acc_z, data.roll, data.pitch, data.yaw) # 将视加速度转化为传感器质心在地惯系下的绝对加速度，并转换单位为m/s^2
    acc_x_data.append(absolute_acc[0]); acc_y_data.append(absolute_acc[1]); acc_z_data.append(absolute_acc[2])
    roll_data.append(data.roll); pitch_data.append(data.pitch); yaw_data.append(data.yaw)
    ...
    if zero_vel:
        # 零速时保持位移不变
        displacement_x_data.append(displacement_x_data[-1] if displacement_x_data else 0)
        displacement_y_data.append(displacement_y_data[-1] if displacement_y_data else 0)
        displacement_z_data.append(displacement_z_data[-1] if displacement_z_data else 0)
    else:
        displacement_x_data.append(x)
        displacement_y_data.append(y)
        displacement_z_data.append(z)
